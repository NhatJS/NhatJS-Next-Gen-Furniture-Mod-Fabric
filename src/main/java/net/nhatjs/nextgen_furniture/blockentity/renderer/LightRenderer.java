...
public class LightRenderer implements BlockEntityRenderer<LightBlockEntity> {
    ...
    private final BakedModel led;

    public LightRenderer(BlockEntityRendererFactory.Context ctx) {
        led = mc.getBakedModelManager().getModel(NhatJSNextGenFurnitureModClient.LIGHT_MODERN_EXTRA);
    }
    ...
    public void render(LightBlockEntity entity, float tickDelta, MatrixStack matrices,
        ...
        matrices.translate(-0.5, -0.5, -0.5);
        bmr.render(matrices.peek(), vc, state, led, 1, 1, 1, light, overlay);
        if (powered) {
            bmr.render(matrices.peek(), vc, state, led, 1, 1, 1, LightmapTextureManager.MAX_LIGHT_COORDINATE, overlay);
        }
        matrices.pop();
    }
}
